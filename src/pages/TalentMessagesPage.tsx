...
// useNotification supprimé - fonctionnalité non implémentée
...
const TalentMessagesPage: React.FC = () => {
  const { user } = useAuth();
  ...
  // Notifications supprimées - fonctionnalité non implémentée
    ...
    try {
      ...
      const success = await FirestoreService.sendMessage(
        ...
        replyContent,
        {
          ...
          email: user.email || '',
          role: user.role,
          displayName: user.displayName || user.email?.split('@')[0] || 'Talent'
        } as any
      );
        ...
        <div style={{ display: 'flex', gap: 24, height: 'calc(100vh - 140px)' }}>
          ...
          <div style={{ 
            ...
            <div style={{ padding: 20, borderBottom: '1px solid #333' }}>
              <h2 style={{ color: '#ffcc00', margin: 0 }}>Boîte de réception</h2>
            </div>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      <h3 style={{ 
                        ...
                        Conversation avec {conversation.interlocutor.name}
                      </h3>
