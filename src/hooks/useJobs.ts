...
export const useJobs = (recruiterId?: string) => {
  ...
  const createJob = async (jobData: Omit<Job, 'id' | 'createdAt' | 'views' | 'applications'>) => {
    setLoading(true);
    setError(null);
...
export const useJobApplications = (jobId?: string, talentId?: string) => {
  ...
  const loadApplications = async () => {
    ...
    try {
      let result: ApiResponse<Application[]>;
    ...
    try {
      ...
      if (result.success) {
        setApplications(prev => prev.map(app => 
          app.id === applicationId ? { ...app, status: status as any, reviewedAt: new Date() } : app
        ));
        return true;
      } else {
        setError(result.error || 'Erreur lors de la mise à jour');
