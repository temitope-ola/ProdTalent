...
export const useAppointments = (userId, userRole) => {
    ...
    const updateAppointmentStatus = async (appointmentId, status) => {
        ...
        try {
            ...
            // Update local state
            setAppointments(prev => prev.map(apt => apt.id === appointmentId ? { ...apt, status: status } : apt));
            return true;
        }
        catch (err) {
    ...
    const upcomingAppointments = appointments.filter(apt => new Date(apt.date) >= new Date() && apt.status === 'confirmé');
