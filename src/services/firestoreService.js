...
export class FirestoreService {
    ...
    static async sendMessage(fromUserId, toUserId, subject, message, fromUserProfile) {
        try {
            ...
            try {
                // Récupérer le profil du destinataire pour son email
                const toUserProfile = await this.getCurrentProfile(toUserId, 'talent'); // ou le bon rôle
                if (toUserProfile && toUserProfile.email) {
                    // Notification email supprimée - fonctionnalité non implémentée
