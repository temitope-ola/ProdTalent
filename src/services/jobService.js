import { doc, getDoc, getDocs, collection, updateDoc, deleteDoc, query, where, addDoc, increment, arrayUnion, Timestamp } from 'firebase/firestore';
...
export class JobService {
    ...
    static async getRecruiterJobs(recruiterId) {
        try {
            const q = query(collection(db, COLLECTION_NAMES.JOBS), where('recruiterId', '==', recruiterId));
    ...
    static async getAllActiveJobs() {
        try {
            const q = query(collection(db, COLLECTION_NAMES.JOBS), where('status', '==', JOB_STATUS.ACTIVE));
            const querySnapshot = await getDocs(q);
            const jobs = querySnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data(),
    ...
    static async getJobById(jobId) {
        try {
            ...
            if (docSnap.exists()) {
                ...
                return createApiResponse(false, null, 'Annonce non trouvée');
            }
        }
        catch (error) {
    ...
    static async getJobApplications(jobId) {
        try {
            const q = query(collection(db, COLLECTION_NAMES.APPLICATIONS), where('jobId', '==', jobId));
            const querySnapshot = await getDocs(q);
    ...
    static async getUserApplications(userId) {
        try {
            const q = query(collection(db, COLLECTION_NAMES.APPLICATIONS), where('talentId', '==', userId));
            const querySnapshot = await getDocs(q);
            const applications = querySnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data(),
